...
rawCapture = cv2.VideoCapture('http://192.168.8.103:8080/video') #= PiRGBArray(camera, size=camera.resolution)
...
while True:
    ...
    gray = frame2[upper_left_y:lower_right_y,upper_left_x:lower_right_x]
    ...
    if base_image is None:
        ...
        lastTime = timestamp
        #frame2.truncate(0)
        cv2.imshow("Speed Camera", frame2)
    ...
    if motion_found:
        if state == WAITING:
            ...
            if state == TRACKING:       
                ...
                with open("spd.data", "w") as text_file:
                    print("{}".format(mph), file=text_file)
