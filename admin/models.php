...
//Speed fine
if ($_GET['do']=='addSpeedFine') { 
	$speed_from=$_POST['speed_from']; 
	$speed_to=$_POST['speed_to']; 

if ($speed_from>$speed_to){
	$message = "Speed input invalid.";
		echo "<script type='text/javascript'>alert('$message');history.go(-1);</script>";
	exit();
}

	$fine_amount=$_POST['fine_amount']; 
	$sql = ("insert into tbl_speed_fine(speed_from,speed_to,fine_amount) values('$speed_from','$speed_to','$fine_amount')");
	(mysqli_query($conn, $sql));
	$message = "Speed fine successfully added.";
	echo "<script type='text/javascript'>alert('$message'); window.location.href='speed-fine-list.php';</script>";
	exit();
}
if ($_GET['do']=='updateSpeedFine') {  
	$id = $_GET['id'];
	$speed_from=$_POST['speed_from']; 
	$speed_to=$_POST['speed_to']; 
	$fine_amount=$_POST['fine_amount']; 
	$sql =("update tbl_speed_fine set speed_from='$speed_from', speed_to='$speed_to', fine_amount='$fine_amount' where id='$id'");
	(mysqli_query($conn, $sql));
	$message = "Speed fine successfully updated.";
	echo "<script type='text/javascript'>alert('$message'); window.location.href='speed-fine-list.php';</script>";
	exit();
}
if ($_GET['do']=='deleteSpeedFine') { 
	$id = $_GET['id']; 
	$sql = "delete from tbl_speed_fine where id = '$id'";
	(mysqli_query($conn, $sql));
	$message = "Speed fine Successfully Deleted.";
	echo "<script type='text/javascript'>alert('$message');window.location.href='speed-fine-list.php';</script>";
	exit();
...
if ($_GET['do']=='activationSubmit') {
	...
	$sql = "update tbl_users_info set stats='$activate_string' where id = '$id'";
	mysqli_query($conn, $sql) ;
