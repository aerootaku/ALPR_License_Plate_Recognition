												...
												<tbody>
													...
													<tr>
														...
														<td><?php if($row['spd_detected'] == '')
														{
															...
															$speed_detected=$row['spd_detected'];
															echo$speedtoKmph=number_format($row['spd_detected']*1.609344,2).'Kmph';
															$speed_fine=mysql_query("SELECT * from tbl_speed_fine WHERE speed_from <= '$speed_detected' AND speed_to  >= '$speed_detected' ")or die(mysql_error());
															if($rowF=mysql_fetch_array($speed_fine)){ 
																$Kmph= $rowF['Kmph'];
																$fine_amount=$rowF['fine_amount']; 
																$id=$row['id']; 
																$sql=("UPDATE tbl_violations SET fee='$fine_amount' WHERE vstatus<>'Paid'");
